...
### Execution logs
- Logs are sanitized through `LogSanitizer` (redaction of sensitive data and truncation).
- Logs are stored in a bounded in-memory buffer (oldest entries trimmed); no persistence to DB or disk (pre‑prod).
- A `Show logs` toggle is visible both while a test is running and after completion to inspect sanitized logs.
